...
class FlutterUnityWidgetController(
    ...
    init {
        ...
        val wasUnityEngineWarm = (UnityPlayerUtils.unityPlayer != null)
        ...
        if (wasUnityEngineWarm) {
            Log.d(LOG_TAG, "Unity engine is already warm in memory. Replaying READY handshake programmatically.")
            Handler(Looper.getMainLooper()).postDelayed({
                methodChannel.invokeMethod("events#onUnityMessage", "READY")
            }, 150) // Safe 150ms delay to let the view container mount and method channels attach
        }
    }
...
//            return UnityPlayerUtils.unityPlayer!!
    ...
    fun reattachToView() {
        val player = UnityPlayerUtils.unityPlayer
        if (player != null && player.parent != view) {
            this.attachToView()
            Handler(Looper.getMainLooper()).post {
