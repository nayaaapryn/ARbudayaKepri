...
#unity-mobile-warning { position: absolute; left: 50%; top: 5%; transform: translate(-50%); background: white; padding: 10px; display: none }
        ...
        private static void ModifyAndroidGradle(bool isPlugin)
        {
            ...
            // Normalize line endings and inject publishing variants block for AGP 8.0+
            buildText = buildText.Replace("useLegacyPackaging true\r\n        }\r\n    }", "useLegacyPackaging true\n        }\n    }");
            buildText = buildText.Replace("useLegacyPackaging true\n        }\n    }", @"useLegacyPackaging true
        }
    }
    ...
    publishing {
        ...
        singleVariant(""debug"") { withSourcesJar() }
            ...
            manifestText = Regex.Replace(manifestText, @"<application .*>", "<application android:extractNativeLibs=\"true\">");
            ...
            // Modify strings.xml to include game_view_content_description required by Unity 6
            var stringsFile = Path.Combine(AndroidExportPath, "src/main/res/values/strings.xml");
            if (File.Exists(stringsFile))
            {
                ...
                {
                    stringsText = stringsText.Replace("</resources>", "  <string name=\"game_view_content_description\" translatable=\"false\">Game view</string>\n</resources>");
        ...
        private static string[] GetEnabledScenes()
        {
            ...
            if (scenes.Length == 0)
            {
                var foundScenes = new[] {
                    ...
                    "Assets/FlutterUnityIntegration/Demo/Scenes/SampleScene.unity",
                    "Assets/FlutterUnityIntegration/Demo/Scenes/scene_2.unity",
                    "Assets/FlutterUnityIntegration/Demo/Scenes/NativeScene.unity"
                }.Where(File.Exists).ToArray();

                if (foundScenes.Length > 0)
                {
                    var newBuildScenes = foundScenes.Select(path => new EditorBuildSettingsScene(path, true)).ToArray();
        ...
        */
        ...
        {
            // Replaced with no-op to protect modern root build.gradle.kts and settings.gradle.kts configurations.
            Debug.Log("[AR Build] Skipping legacy SetupAndroidProject configuration (Kotlin DSL protective lock enabled).");
        }
        ...
        {
            // Replaced with no-op to protect modern root build.gradle.kts and settings.gradle.kts configurations.
            Debug.Log("[AR Build] Skipping legacy SetupAndroidProjectForPlugin configuration (Kotlin DSL protective lock enabled).");
        }
